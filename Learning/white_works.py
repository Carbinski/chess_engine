...
class BasicSearchBot:
    ...
    def orderMoves(self, moves: List[chess.Move]) -> List[chess.Move]:
        ...
        for move in moves:
            ...
            # Take to long for now...

            # # Penalize for moving into an attacked square
            # if self.board.is_attacked_by(color= not self.board.turn, square=move.to_square):
            #     score -= 2 * self.piece_to_points(self.board.piece_at(move.from_square))

            # # Bonus for moving into a supported square:
            # if self.board.is_attacked_by(color=self.board.turn, square=move.to_square):
                score += self.piece_to_points(self.board.piece_at(move.from_square))
    ...
    def search(self, depth: int, alpha: int, beta: int, maximizing_player: bool) -> int:
        """
        ...
        if maximizing_player:
            ...
            for move in moves:
                ...
                eval = self.search(depth - 1, alpha, beta, False)
                self.board.pop()
            ...
            for move in moves:
                self.board.push(move)
                eval = self.search(depth - 1, alpha, beta, True)
                self.board.pop()
