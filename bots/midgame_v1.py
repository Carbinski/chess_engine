...
class MidGameBotV1:
    ...
    def delta_evaluate(self, move) -> int:
        ...
        mg_phase = self.get_phase()
        eg_phase = 1 - mg_phase

        val, mg_tbl, eg_tbl = PIECE_WT[piece.piece_type]

        if piece.color:
            delta -= (val + (mg_phase * mg_tbl[source]) + (eg_phase * eg_tbl[source]))
        else:
            delta -= -(val + (mg_phase * mg_tbl[source ^ 56]) + (eg_phase * eg_tbl[source ^ 56]))

        if move.promotion:
            p_val, p_mg_tbl, p_eg_tbl = PIECE_WT[move.promotion]
            if piece.color:
                delta += (p_val + (mg_phase * p_mg_tbl[target]) + (eg_phase * p_eg_tbl[target]))
            else:
                delta += -(p_val + (mg_phase * p_mg_tbl[target ^ 56]) + (eg_phase * p_eg_tbl[target ^ 56]))
        else:
            if piece.color:
                delta += (val + (mg_phase * mg_tbl[target]) + (eg_phase * eg_tbl[target]))
            else:
                delta += -(val + (mg_phase * mg_tbl[target ^ 56]) + (eg_phase * eg_tbl[target ^ 56]))
        ...
        if board.is_capture(move):
            if board.is_en_passant(move):
                ...
                if ep_piece:
                    c_val, c_mg_tbl, c_eg_tbl = PIECE_WT[ep_piece.piece_type]
                    if ep_piece.color == chess.WHITE:
                        delta -= (c_val + (mg_phase * c_mg_tbl[cap_sq]) + (eg_phase * c_eg_tbl[cap_sq]))
                    else:
                        delta -= -(c_val + (mg_phase * c_mg_tbl[cap_sq ^ 56]) + (eg_phase * c_eg_tbl[cap_sq ^ 56]))
            else:
                captured_piece = board.piece_at(target)
                if captured_piece:
                    c_val, c_mg_tbl, c_eg_tbl = PIECE_WT[captured_piece.piece_type]
                    if captured_piece.color == chess.WHITE:
                        delta -= (c_val + (mg_phase * c_mg_tbl[target]) + (eg_phase * c_eg_tbl[target]))
                    else:
                        delta -= -(c_val + (mg_phase * c_mg_tbl[target ^ 56]) + (eg_phase * c_eg_tbl[target ^ 56]))
        ...
        if piece.piece_type == chess.KING and abs(source - target) == 2:
            ...
            r_val, r_mg_table, r_eg_table = PIECE_WT[chess.ROOK]

            if self.board.turn == chess.WHITE:
                delta -= (r_val + (mg_phase * r_mg_table[rook_from]) + (eg_phase * r_eg_table[rook_from]))
                delta += (r_val + mg_phase * r_mg_table[rook_to] + (eg_phase * r_eg_table[rook_to]))
            else:
                delta -= -(r_val + mg_phase * r_mg_table[rook_from ^ 56] + (eg_phase * r_eg_table[rook_from ^ 56]))
                delta += -(r_val + mg_phase * r_mg_table[rook_to ^ 56] + (eg_phase * r_eg_table[rook_to ^ 56]))
    ...
    def search(self, depth: int, alpha: int, beta: int, current_eval: int) -> int:      
        ...
        max_eval = float("-inf")
        push, pop, get_delta = self.board.push, self.board.pop, self.delta_evaluate
